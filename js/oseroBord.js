var board = []

function NewBoard() {
    // [i][j]ã®ä½ç½®ã«nextã¨colorã®ãƒ‡ãƒ¼ã‚¿ã‚’å…¥ã‚Œã‚‹.
    for (let i = 0; i < 8; i++) {
        let tmpA = []
        for (let j = 0; j < 8; j++) {
            // nextã¯æ¬¡ã®æ‰‹ã®ä½ç½®, colorã¯ã‚³ãƒžã®è‰²
            if ((i == 3 || i == 4) && i == j) {
                tmpA.push({
<<<<<<< HEAD
                    next: null.
                    color: "white".
                    canset: false
=======
                    next: null,
                    color: "white"
>>>>>>> upstream/master
                })
            }
            else if ((i == 3 && j == 4) || (j == 3 && i == 4)) {
                tmpA.push({
<<<<<<< HEAD
                    next: null.
                    color: "black".
                    canset: false
=======
                    next: null,
                    color: "black"
>>>>>>> upstream/master
                })
            }
            else {
                tmpA.push({
<<<<<<< HEAD
                    next: null.
                    color: "".
                    canset: false
=======
                    next: null,
                    color: ""
>>>>>>> upstream/master
                })
            }
        }
        board.push(tmpA)
    }
}
<<<<<<< HEAD
=======

//ƒRƒ}‚ð’u‚­
function placeDisk(xPos, yPos, color) { //¶ã xPos = 0, yPos = 0;
    board[yPos][xPos].color = color;
    
    
    let sPos = {x:xPos, y:yPos}; //’T‚·ˆÊ’u

    let rColor; //’u‚­ƒRƒ}‚Æ”½‘Î‚ÌF
    (color == "white") ? rColor = "black" : rColor = "white";

    let count = 0;
    let stack = []; //‚Ð‚Á‚­‚è•Ô‚·ƒRƒ}‚ÌˆÊ’u‚ð“ü‚ê‚é

    let dir = [
        {x:0,y:-1}, //ã
        {x:0,y:1}, //‰º
        {x:-1,y:0}, //¶
        {x:1,y:0}, //‰E
        {x:1,y:-1}, //‰Eã
        {x:-1,y:-1}, //¶ã
        {x:1,y:1}, //‰E‰º
        {x:-1,y:1} //¶‰º
    ]; 
    
    
    //‚Ð‚Á‚­‚è•Ô‚·ƒRƒ}‚ðstack‚É“ü‚ê‚é
    for (let n = 0; n < dir.length; n++) {
        sPos.x = xPos;
        sPos.y = yPos;

        let flag = false; //while ‚ð”²‚¯‚éƒtƒ‰ƒbƒO;
        while (true) {
            sPos.x += dir[n].x;
            sPos.y += dir[n].y;

            if (sPos.y < 0 || 7 < sPos.y || sPos.x < 0 || 7 < sPos.x) {
                flag = true;
            }
            if (flag === true) {
                for (let i = 0; i < count; i++) {
                    stack.pop();
                }
                break;
            }

            if (board[sPos.y][sPos.x].color == rColor) {
                stack.push(Object.assign({}, sPos));
                count++;
            } else if (board[sPos.y][sPos.x].color == color){
                flag = true;
                count = 0;
            } else {
                flag = true;
            }

            
        }
        count = 0;
    }

    //‚Ð‚Á‚­‚è•Ô‚·
    while (stack.length != 0) {
        let temp = stack.pop();
        board[temp.y][temp.x].color = color;
    }
}
>>>>>>> upstream/master
